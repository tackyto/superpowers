...
|---|---|---|
| Native plugin marketplace | Claude Code | Register in `.claude-plugin/marketplace.json`; users `/plugin install`. In this fork that file *is* the marketplace users add (`/plugin marketplace add tackyto/superpowers`, then `/plugin install superpowers@superpowers-tackyto`) — upstream instead publishes through its separate `obra/superpowers-marketplace` repo. |
