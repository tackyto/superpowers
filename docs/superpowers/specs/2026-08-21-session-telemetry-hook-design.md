...
各 `append_records` 呼び出しは、月ごとに 1 回のファイル排他ロックを取得し、
そのバッチ全体を一度に `write` する。ロックが即座に取れなければ、
約 50ms を基準にプロセスごとの jitter を加えて 12 回までリトライする。
（jitter は `LOCK_WAIT_SECONDS * (1 + (os.getpid() % 7) / 7.0)` で、
最大待機時間は約 1.0 秒。10 秒のフックタイムアウトに対して十分な余裕がある）
それでも取得できなければ `errors.log` に落として諦める。

**損失の単位はバッチであり、単一行ではない。**
`append_records` は複数行を束ねて 1 回のロック操作で追記するため、
ロック取得失敗により失われるのはそのバッチ全体である。
この代価は、セッションを止めないという原則より小さいため、
素早く諦めることを優先する。
