...
`errors.log` に `session` が `"unknown"` のまま `JSONDecodeError` が入っている場合、
壊れているのは transcript ではなくフックに渡されたペイロードの方である。2026-08-27
より前のバージョンは stdin を Python の既定エンコーディングで読んでいたため、日本語
Windows(cp932)では非 ASCII を含むペイロードが壊れることがあった。cp932 の lead byte
60 種のうち 52 種が直後の `\` を飲み込むので、`\"` が `"` になって文字列がそこで
終わってしまう。現在は stdin をバイト列として読み UTF-8 で明示的にデコードしている。

このエラーが出ても記録は失われない。`run()` は追記に成功してから状態を保存するので、
失敗した回は読み取り位置が進まず、次に成功した回が取りこぼし分をまとめて処理する。
