...
| `AGENTS.md` | The symlink to `CLAUDE.md` replaced with a real file that points at `CLAUDE.md` and restates the never-PR-upstream rule in full | Git on Windows cannot reproduce a symlink without Developer Mode. Measured on Windows 11 with Developer Mode off: a default clone writes a 9-byte regular file containing the string `CLAUDE.md` and reports a **clean** worktree, while `git clone -c core.symlinks=true` leaves the file missing entirely and the worktree permanently showing ` D AGENTS.md`. Either way the file that forbids PRs against upstream quietly stopped being readable on the platform. The rule is restated rather than only linked so an agent that reads no further still gets it. | Low — upstream is unlikely to edit a symlink, but if a merge reintroduces mode `120000`, keep ours. `tests/repo/test-agent-instructions.sh` fails if it comes back, or if any other tracked file becomes a symlink. |
